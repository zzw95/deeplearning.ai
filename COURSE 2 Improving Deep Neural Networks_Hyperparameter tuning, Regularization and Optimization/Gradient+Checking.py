...
def gradient_check(x, theta, epsilon = 1e-7):
    ...
    gradapprox = (J_plus-J_minus)*0.5/epsilon                             # Step 5
    ### END CODE HERE ###
...
def backward_propagation_n(X, Y, cache):
    ...
    dW2 = 1./m * np.dot(dZ2, A1.T)                #1./m * np.dot(dZ2, A1.T) * 2
    db2 = 1./m * np.sum(dZ2, axis=1, keepdims = True)
    
    dA1 = np.dot(W2.T, dZ2)
    dZ1 = np.multiply(dA1, np.int64(A1 > 0))
    dW1 = 1./m * np.dot(dZ1, X.T)                
    db1 = 1./m * np.sum(dZ1, axis=1, keepdims = True)  # 4./m * np.sum(dZ1, axis=1, keepdims = True)
...
def gradient_check_n(parameters, gradients, X, Y, epsilon = 1e-7):
    ...
    for i in range(num_parameters):
        ...
        ### START CODE HERE ### (approx. 1 line)
        gradapprox[i] = (J_plus[i]-J_minus[i])*0.5/epsilon
        ### END CODE HERE ###
